...
6) ___Rations___ Crafting a ration will sate anyone who eats its (up to the GM's discretion as to *how many* characters can be fed from a crafting). Without a recipe or exotic ingredients (i.e. components), rations merely grant a number of temporary HP. Many of the components possess the infuser tag, which applies a component effect to the crafted ration. These effects can have an immediate effect or last for a duration specified on the generated recipe file per the discretion of the GM.
	- In combat, you can use your action to either feed yourself or a friendly creature within 5 feet one ration. 

	- Friendly creatures can only be under the effect of one ration at a time, and consuming a new ration immediately ends any current ration effects. 
